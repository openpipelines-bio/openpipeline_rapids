// tsne build_harmony_leiden_benchmark
// 
...
meta = [
  ...
  "description" : "t-SNE (t-distributed Stochastic Neighbor Embedding) is a non-linear\ndimensionality reduction technique for visualizing high-dimensional data.\nIt computes a low-dimensional embedding from a precomputed representation\nin `.obsm` (e.g. `X_pca`) and writes the result to `.obsm`. Wraps the\n`rapids_singlecell.tl.tsne` function.\n",
  "test_resources" : [
    {
      "type" : "python_script",
      "path" : "test.py",
  ...
  "build_info" : {
    "config" : "/home/runner/work/openpipeline_rapids/openpipeline_rapids/src/tools/tsne/config.vsh.yaml",
    "runner" : "nextflow",
    "engine" : "docker",
...
par = {
  ...
  'perplexity': $( if [ ! -z ${VIASH_PAR_PERPLEXITY+x} ]; then echo "int(r'${VIASH_PAR_PERPLEXITY//\\'/\\'\\"\\'\\"r\\'}')"; else echo None; fi ),
  'early_exaggeration': $( if [ ! -z ${VIASH_PAR_EARLY_EXAGGERATION+x} ]; then echo "float(r'${VIASH_PAR_EARLY_EXAGGERATION//\\'/\\'\\"\\'\\"r\\'}')"; else echo None; fi ),
...
meta["defaults"] = [
  ...
  "container" : {
    "registry" : "ghcr.io",
    "image" : "openpipelines-bio/openpipeline_rapids/tools/tsne",
